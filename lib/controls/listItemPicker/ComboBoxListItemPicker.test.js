...
var ComboBoxListItemPicker_1 = require("./ComboBoxListItemPicker");
var RequestClientMock_1 = require("../../common/mocks/RequestClientMock");
var mockHttpClient = new RequestClientMock_1.RequestClientMock(null);
...
describe('<ComboBoxListItemPicker />', function () {
    it("Should render initial data", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", filter: "Id gt 0", listId: "TestId", onInitialized: function () {
                    chai_1.expect(comboBox.state('availableOptions')).to.have.length(10);
                    resolve();
    ...
    it("Should call onSelectedItem", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", filter: "Id gt 0", listId: "TestId", onInitialized: function () {
                    ...
                    chai_1.expect(item.Id).to.equal(4);
                    resolve();
                } }));
        });
    });
    it("Should initialize with default selection (id)", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", defaultSelectedItems: [1], filter: "Id gt 0", listId: "TestId", onInitialized: function () {
                    var ddInput = comboBox.find('.ms-ComboBox-Input').first();
                    chai_1.expect(ddInput.getNode().value).to.be.equal("Test 1");
                    resolve();
                }, onSelectedItem: function (item) {
                } }));
        });
    ...
    it("Should initialize with default selection (object)", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", defaultSelectedItems: [{ Id: 1, Title: "Test 1" }], filter: "Id gt 0", listId: "TestId", onInitialized: function () {
                    var ddInput = comboBox.find('.ms-ComboBox-Input').first();
                    chai_1.expect(ddInput.getNode().value).to.be.equal("Test 1");
                    resolve();
                }, onSelectedItem: function (item) {
    ...
    it("Should call onSelectedItem (multi)", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", filter: "Id gt 0", listId: "TestId", multiSelect: true, onInitialized: function () {
                    var ddBtn = comboBox.find('.ms-Button-flexContainer').first();
                    ddBtn.simulate('click');
                    //actual list is not part of the component
                    var checkBoxBtn = document.querySelectorAll('.ms-ComboBox-option')[3];
                    ...
                    chai_1.expect(item.Id).to.equal(4);
                    chai_1.expect(item.selected).to.be.equal(true);
                    var ddBtn = comboBox.find('.ms-Button-flexContainer').first();
                    ddBtn.simulate('click');
                    resolve();
                } }));
    ...
    it("Should initialize with default selection (multi) (object)", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", defaultSelectedItems: [{ Id: 1, Title: "Test 1" }, { Id: 2, Title: "Test 2" }], filter: "Id gt 0", listId: "TestId", multiSelect: true, onInitialized: function () {
                    var ddBtn = comboBox.find('.ms-Button-flexContainer').first();
                    ddBtn.simulate('click');
                    var checkBoxBtn = document.querySelectorAll('.ms-ComboBox-option')[0];
                    chai_1.expect(checkBoxBtn.classList.contains("is-checked")).to.be.equal(true);
                    var ddInput = comboBox.find('.ms-ComboBox-Input').first();
                    chai_1.expect(ddInput.getNode().value).to.be.equal("Test 1, Test 2");
                    ddBtn.simulate('click');
                    resolve();
                }, onSelectedItem: function (item) {
                } }));
    ...
    it("Should initialize with default selection (multi) (id)", function () {
        return new Promise(function (resolve, error) {
            var comboBox = enzyme_1.mount(React.createElement(ComboBoxListItemPicker_1.ComboBoxListItemPicker, { columnInternalName: "Title", spHttpClient: mockHttpClient, webUrl: "/sites/test-site", defaultSelectedItems: [1, 2], filter: "Id gt 0", listId: "TestId", multiSelect: true, onInitialized: function () {
                    var ddBtn = comboBox.find('.ms-Button-flexContainer').first();
                    ddBtn.simulate('click');
                    var checkBoxBtn = document.querySelectorAll('.ms-ComboBox-option')[0];
                    chai_1.expect(checkBoxBtn.classList.contains("is-checked")).to.be.equal(true);
